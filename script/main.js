// Função do contador crescente a partir de 8 de abril de 2025
function iniciarContador() {
  const dataInicio = new Date("2025-04-08T00:00:00");

  function atualizarContador() {
    const agora = new Date();
    const diferencaSegundos = Math.floor((agora - dataInicio) / 1000);
    ...
    const texto = `Tempo juntos:\n${dias} Dias, ${String(horas).padStart(2, '0')} Horas, ${String(minutos).padStart(2, '0')} Minutos e ${String(segundos).padStart(2, '0')} Segundos`;
  ...
  const contador = document.getElementById("contador");
  if (contador && contador.classList.contains("oculto")) {
...
const animationTimeline = () => {
  ...
  tl.to(".container", 0.1, { visibility: "visible" })
    ...
    .to({}, 0, { onComplete: iniciarContador }); // ⏱ Inicia o contador após a parte .nine
...
// Carrega dados personalizados e executa a animação
const fetchData = () => {
